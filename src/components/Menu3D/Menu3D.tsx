...
const Menu3D = () => {
    const menu = useMemo(() => {
        ...
        return (<ul> {[{id: 8, href: "/", caption: "Home"}, {id: 7, href: "/login", caption: "Login"}, {
        ...
        }, {id: 3, href: "/glow", caption: "Glow checkbox"}, {id: 2, href: "/kbd", caption: "Keyboard"}, {
            id: 1, href: "/clock", caption: "Clock",
        }].map(item => <li key={item.id.toString()} style={idStyle(item.id)}><a href={item.href}>{item.caption}</a>
        </li>)}</ul>);
    }, []);
